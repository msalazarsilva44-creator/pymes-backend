...
<head>
    ...
    <title>Reporte de Ventas - Admin MERCAROF</title>
...
<body class="main-bg">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center gap-4">
                    <a href="/dashboard/admin" class="text-xl font-bold text-mercarof-navy">MERCAROF</a>
                    <span class="text-gray-400">/</span>
                    <span class="text-gray-600 font-medium">Reporte de Ventas</span>
                </div>
                <a href="/dashboard/admin" class="text-gray-600 hover:text-mercarof-cyan transition-colors font-medium text-sm flex items-center gap-2">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-white/80">Todas las órdenes del marketplace</p>
                    </div>
                </div>
                <div class="flex gap-3 flex-wrap">
                    <!-- Filtro estado -->
                    <select id="filtro-estado" onchange="cargarVentas()" class="px-4 py-2 bg-white/20 backdrop-blur-sm text-white border border-white/30 rounded-xl focus:ring-2 focus:ring-white/50 cursor-pointer">
                        ...
                        <option value="pagado" class="text-gray-900">💳 Pagados</option>
                    ...
                    <!-- Items por página -->
                    <select id="items-por-pagina" onchange="cargarVentas()" class="px-4 py-2 bg-white/20 backdrop-blur-sm text-white border border-white/30 rounded-xl focus:ring-2 focus:ring-white/50 cursor-pointer">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-white rounded-xl shadow-sm p-5 text-center border-l-4 border-mercarof-cyan">
                <p class="text-3xl font-bold text-mercarof-navy" id="stat-volumen">$0</p>
                <p class="text-sm text-gray-500 mt-1">Volumen Total</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody id="tabla-ventas" class="divide-y divide-gray-100">
                        <tr>
    ...
    <script>
        ...
        async function cargarVentas() {
            ...
            try {
                const response = await fetch(`${API_URL}/admin/reportes/ventas?estado=${filtro}`, {
                    headers: {
                ...
                if (result.success) {
                    ...
                    // Actualizar stats
                    document.getElementById('stat-total').textContent = result.stats.total_ordenes;
                    document.getElementById('stat-completadas').textContent = result.stats.ordenes_completadas;
                    document.getElementById('stat-pendientes').textContent = result.stats.ordenes_pendientes;
                    document.getElementById('stat-volumen').textContent = '$' + parseFloat(result.stats.volumen_total || 0).toFixed(2);
                ...
                document.getElementById('tabla-ventas').innerHTML = '<tr><td colspan="7" class="px-4 py-12 text-center text-red-500">Error al cargar ventas</td></tr>';
            }
        }
        ...
        function renderizarTabla() {
            ...
            const ventasPagina = todasLasVentas.slice(inicio, fin);
            
            totalPaginas = Math.ceil(todasLasVentas.length / itemsPorPagina);
            ...
            if (ventasPagina.length === 0) {
                ...
                tbody.innerHTML = ventasPagina.map(orden => `
                    <tr class="hover:bg-gray-50 cursor-pointer transition-colors" onclick="verOrden(${orden.id})">
                        <td class="px-4 py-4">
                            <span class="font-mono text-sm text-gray-600">${orden.numero_orden}</span>
                        </td>
                        <td class="px-4 py-4">
                            <p class="font-semibold text-gray-900">${orden.empresa?.nombre_comercial || 'N/A'}</p>
                        </td>
                        <td class="px-4 py-4">
            ...
            const infoInicio = todasLasVentas.length > 0 ? inicio + 1 : 0;
            const infoFin = Math.min(fin, todasLasVentas.length);
            document.getElementById('info-paginacion').textContent = `Mostrando ${infoInicio}-${infoFin} de ${todasLasVentas.length} órdenes`;
        ...
        function verOrden(ordenId) {
            const orden = todasLasVentas.find(o => o.id === ordenId);
            if (!orden) return;
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <p class="text-xs text-gray-500 mb-1">Empresa</p>
                            <p class="font-bold text-gray-900">${orden.empresa?.nombre_comercial || 'N/A'}</p>
                        </div>
                        <div class="p-4 bg-gray-50 rounded-xl">
                            <p class="text-xs text-gray-500 mb-1">Cliente</p>
                            <p class="font-bold text-gray-900">${orden.user?.name || 'N/A'}</p>
                            <p class="text-xs text-gray-600">${orden.user?.email || ''}</p>
                        </div>
                    </div>

                    <div>
                        <p class="text-xs text-gray-500 mb-2">Servicios (${orden.items?.length || 0})</p>
                        <div class="space-y-2">
